...
const fixtureRoot = join(packageRoot, 'fixtures/trpc-consumer')
...
const temporaryRoot = await mkdtemp(join(tmpdir(), 'gbl-platform-consumer-'))
...
try {
  ...
  manifest.dependencies['@gbl-uzh/platform'] = `file:${packageArchive}`
  ...
  execFileSync(process.execPath, [pnpmCli, 'run', 'check'], {
    cwd: consumerRoot,
    env: environment,
    stdio: 'inherit',
  })
  execFileSync(process.execPath, [pnpmCli, 'run', 'smoke'], {
    cwd: consumerRoot,
    env: environment,
    stdio: 'inherit',
  })

  console.log(`External tRPC consumer verified from ${packageArchive}`)
} finally {
