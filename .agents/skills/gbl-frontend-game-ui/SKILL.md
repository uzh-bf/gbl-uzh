...
3. **Shared chrome** in a `GameLayout` wrapper: nav + team identity + Ready toggle + countdown + learning activities + blocking story-element popups. The demo game uses bottom tabs at `/play/cockpit?tab=cockpit|market|history|team`: the first tab is labeled Decisions while retaining its `cockpit` URL key; Ready appears there only during RUNNING. Profile, XP, settled portfolio statistics, learning activities, and released stories live in Team, Market shows scenario probabilities and admin-revealed monthly outcomes, and History shows cumulative portfolio values with year-filtered expandable quarterly results. Other games may retain a sidebar.
...
The demo-game Market uses `ProbabilityChart variant="market"`: all forecast values use the active scenario; only actual admin-revealed totals are highlighted. Dice beneath the asset titles and the returns comparison use the latest revealed historical result. Dice are fixed at segment creation; admin animation persists `revealedRollIndices` and emits `MARKET_ROLL_REVEALED` to trigger a result refetch. Reveals are allowed during allocation and must preserve dirty drafts. Closing a quarter does not automatically reveal its rolls.

## The two chart layers (PAUSED vs RESULTS)

The demo-game History tab is independent of the Decisions result screens. `src/lib/results.ts` contains `buildHistory`, `buildResultView`, and their shared settlement, sample, allocation, initial-capital, numeric, and formatting helpers; they use the aggregate query and lifecycle state to exclude unfinished `SEGMENT_END` records (these exist before settlement). `PERIOD_END` records identify finished years, including when RESULTS points to an upcoming year or disconnects the final pointer. Use persisted `assetsWithReturns` for balances/monthly CHF changes and compound monthly market rates for quarterly returns; result-facts asset changes can include reallocation. The year selector filters only the table, preserving the cumulative summary/chart. Expanded months show dice only for `revealedRollIndices`; quarter completion does not reveal dice. Use `AllocationBar compact` for the small mix with an accessible percentage label on its wrapper. Test cross-year/final results, live settlement/reveals, year selection, and draft retention.
...
- **Story elements** (blocking narrative popups): shown at segment activation, before the player decides. They contextualise the round ("A supply shock has hit the market…") and build the game's narrative arc. Seeded as `StoryElement` rows per segment. Other games render with `StoryElements` from `@gbl-uzh/ui`; it blocks interaction until dismissed. Demo-game uses app-local `team/TeamContent` and `StorySheet` to add historical rereading: Continue persists Read; Close/Escape/Skip dismiss the automatic sequence for the page visit without marking skipped cards Read. Library entries retain first-release metadata and exclude future segments.
- **Learning elements** (Team tab in demo-game; sidebar in other games): optional MC questions or reflection prompts available during play. They reinforce the theory behind the game mechanic ("According to the Phillips Curve, what happens to unemployment when inflation rises?"). Seeded as `LearningElement` rows. Compose `LearningActivitiesList`, `LearningActivityModal`, and `useLearningActivities` from `@gbl-uzh/ui` with the game's generated GraphQL documents. Demo-game uses local `TeamPanel`/`LearningSheet` presentation with the shared hook's opt-in `preserveDrafts`, query retry, and submission errors. Viewed IDs are browser-local per game/team (New → Open); Solved remains server-backed. The countdown is advisory. Show only configured `reward.xp`, not screenshot rewards.
...
Use colocated Tailwind utilities for player layouts and states. The demo-game `src/globals.css` defines shared `player-*` color/font tokens; welcome and cockpit use these tokens throughout, with typography applied directly to portaled picker content. Welcome uses app-local `WelcomeActionButton`, `WelcomeTextButton`, and `WelcomeTextInput` wrappers that preserve native props and refs; its actions keep a 44px mobile / 48px desktop minimum height and content/full-width sizing. Preserve the welcome shell’s 720px cap, 359px avatar-grid boundary, 721px desktop boundary, and safe-area padding. Reuse app-local `PlayerActionButton`, `AllocationNotice`, `AllocationRow`, and `AllocationBar` before duplicating cockpit styling. Preserve explicit pixel dimensions and breakpoints: the app root is 14px, so rem-based utility substitutions can change the design. Keep dynamic slider percentages inline and custom CSS limited to browser-specific resets. Pass overrides through design-system class slots instead of blanket `!important` selectors. See the player styling convention in `docs/ui-components.md`.
