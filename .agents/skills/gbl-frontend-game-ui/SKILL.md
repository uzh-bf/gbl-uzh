...
- For a mutation that drives a critical transition, arm waits for its exact tRPC POST request and response before clicking. Retry the click only until that request is observed, then require `response.ok()` before asserting durable UI state.
