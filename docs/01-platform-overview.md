...
| `packages/ui`                                | `@gbl-uzh/ui`        | Shared game UI components (layout, nav, timeline, XP bar, ...) — early stage, see [06-ui-components.md](06-ui-components.md) |
...
**Practical consequence:** the supported way to build a new game today is inside a clone/fork of this monorepo as a new `apps/<your-game>` workspace package. Building against the npm package alone is untested and would lose `@gbl-uzh/ui`. There is no scaffolding CLI; you copy `apps/demo-game` (see [05-developing-a-game.md](05-developing-a-game.md)).
...
- Realtime pub/sub so clients refresh when the admin advances the game ([07-api-layer.md](07-api-layer.md))
...
| Decision | Player input — two distinct channels (in-segment actions vs. explicit period decisions), see [02-game-model.md](02-game-model.md). |
| Result   | A computed per-player snapshot at each boundary (period/segment start/end) — the input for charts & reports.                       |

Deeper definitions and the full entity list: [02-game-model.md](02-game-model.md).
