...
| Demo-game login unexpectedly opens real Auth0                                                        | An ignored `.env.local` opts demo-game into Auth0                                                                                            | Leave `GBL_AUTH_MODE` unset (or set it to `mock`) for local mock login; use `GBL_AUTH_MODE=auth0` only when intentionally testing a configured Auth0 tenant.                                                                                                                                                |
