...
| Login unexpectedly opens real Auth0                                                                  | An ignored `.env.local` contains old `AUTH0_*` values                                                                                        | Leave `GBL_AUTH_MODE` unset (or set it to `mock`) for local mock login; use `GBL_AUTH_MODE=auth0` only when intentionally testing a configured Auth0 tenant.                                                                                                                                                |
