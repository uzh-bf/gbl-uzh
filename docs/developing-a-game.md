...
tags:
  ...
  - frontend
  - scaffolding
timestamp: "2026-09-23T12:34:52Z"
---
...
The existing seed sets `{ xp: LEARNING_ACTIVITY_XP }` on the four demo activities, using the shared value of 20 in `src/lib/constants.ts`. Other games retain their configured lesson rewards. `PlayService.attemptLearningElement` records first completion and its reward atomically, using the existing completion IDs/relation to prevent repeats. No schema migration or separate list of rewarded lessons is required.
