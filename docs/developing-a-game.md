...
- **Responsive sizing** (`apps/demo-game/src/components/cockpit/Cockpit.module.css`): the single-column shell stays centered and capped at 784px. Editing, submitted, and Ready share header, progress, footer, navigation, and action-button dimensions at each viewport. Above 600px, spacing and headings increase modestly; Submit and Change allocation share one size rule in enabled and disabled states.
...
- **Summary styling** (`apps/demo-game/src/components/cockpit/Cockpit.module.css`): allocation labels are centered within each colored section, Bonds uses the same navy in editing and saved summaries, the active progress segment has no outer halo, and the checked Ready switch uses the green readiness color.
- **Submitted and Ready** (`apps/demo-game/src/components/cockpit/AllocationSummary.tsx:AllocationSummary`): submission replaces the form with a persisted, read-only mix and CHF breakdown. Change allocation reopens saved percentages; Ready is disabled in the editor until resubmission. Ready locks the cockpit controls, keeps the countdown visible, and can be switched off to return to the summary. The disabled Change allocation button retains its normal width. Market and History remain accessible placeholders. The lock is UI-only; the shared API remains advisory and the instructor still closes each segment.
