...
- The `GBL Platform` (located in the `packages/platform` directory), a code framework for building round-based simulations with Next.js and React. Documentation for building games on the platform (written for humans and AI coding agents) lives in [`docs/`](docs/README.md), with matching agent skills in `.agents/skills/`.
