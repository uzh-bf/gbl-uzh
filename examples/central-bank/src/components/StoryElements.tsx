...
function StoryElements({ playerState, player }: Props) {
  ...
  useEffect(() => {
    if (activeStoryElements?.length > 0) {
      const unseenStoryElements = activeStoryElements.filter(
        (elem: any) => !visitedStoryElements?.includes(elem.id)
      );
      setUnseenStoryElements(unseenStoryElements as any);
    }
  ...
  const content: string = (() => {
    ...
    const firstElement = unseenStoryElements[0] as any;
  ...
  return (
    <Modal
      ...
      onPrimaryAction={() => {
        markStoryElement({
          variables: {
            elementId: (unseenStoryElements[0] as any)?.id,
          },
          // optimisticResponse: {
          //   markStoryElement: {
      ...
      title={(unseenStoryElements[0] as any)?.title}
    >
      <div>
        <Progress
