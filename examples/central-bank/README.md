...
Local development through either devcontainer uses mocked OIDC, not real Auth0.
The committed environments point `GBL_MOCK_OIDC_ISSUER` at the local
`mock-oauth2-server` sidecar, so admin login works without real credentials.
